...
namespace PurrfectEngine {
  ...
  void vkTexture::initialize(vkCommandPool *pool, vkDescriptorPool *descriptors, VkFormat format, VkImageLayout targetLayout, int width, int height, bool mipmaps, bool msaaSamples, bool descriptor, uint8_t *pixelS) {
    ...
    bool isTransDst = false;
    {
      ...
      if (mFilename && !pixels) {
        int texChannels;
        pixels = stbi_load(mFilename, &texWidth, &texHeight, &texChannels, STBI_rgb_alpha);
